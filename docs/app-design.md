...
- **3 つのフローの違いを体感できる**: Interactive / Autonomous App / Autonomous User それぞれで「誰の資格情報でリソース API が呼ばれたか」を同一画面で比較参照できる
...
- **技術的な根拠**: Agent Identity は `client_credentials` (Autonomous App)、`user_fic` (Autonomous User)、`jwt-bearer` OBO (Interactive) の**複数の grant type をサポートする**。公式ドキュメントにも「Agent identities use `client_credentials` for app-only autonomous operations and the `jwt-bearer` grant type supports both client credential flow and OBO flow, providing **flexibility in delegation patterns**」と明記されている。なお Autonomous User の Agent User トークン取得は OBO（`jwt-bearer` grant）ではなく、専用の `user_fic` grant type を使用する。
- **3 シナリオで異なる caller が見える理由はトークンの subject**: 同一 Agent Identity が 3 フローを実行しても、リソース API は完全に異なる呼び出し元を認識する（Interactive → 人間ユーザーの sub/upn、Autonomous App → Agent Identity の appId が subject、Autonomous User → Agent User の upn）。Agent Identity を複数用意する必要はない。
...
graph TB
    ...
    subgraph "Microsoft Foundry"
        FA["🤖 Foundry Hosted Agent<br/>- Entra Agent ID 対応<br/>- System-Assigned MSI (Foundry 自動管理)<br/>- LLM (GPT-4o)"]
    end
...
sequenceDiagram
    ...
    EID-->>FE: ③ Tc (Blueprint scope)<br/>+ Foundry API token (Cognitive Services scope)
    FE->>FA: ④ Foundry Agent API 呼び出し<br/>[Authorization: Bearer {Foundry API token}]<br/>[message: "...Tc={Tc_value}..."]
    ...
    DA-->>FA: ⑧ {callerType: "delegated_human_user",<br/>callerUpn: "alice@contoso.com"}
    FA-->>FE: ⑨ LLM 整形レスポンス
    FE-->>U: ⑩ 結果表示
```

> **ポイント**: Foundry Agent API を呼び出すのは**ユーザー本人**（MSAL で取得した Entra ID トークン）。リソース API への最終的な TR も同じユーザーを sub に持つ。Tc はメッセージペイロードに含めて Function Tool に引き渡す。
...
sequenceDiagram
    ...
    FE->>BE: ② POST /api/demo/autonomous/{app|user}<br/>（認証トークン不要、フローパラメータのみ）
    ...
    Note over FA: Function Tool 実行
    FA->>EID: ④ T1 取得 (Project System-Assigned MSI)
    alt Autonomous App Flow
        FA->>EID: ⑤a client_credentials (T1 → TR)<br/>sub = Agent Identity OID
    else Autonomous User Flow
        FA->>EID: ⑤b user_fic (T1 → T2 → TR)<br/>sub = agentuser@contoso.com
    end
    FA->>DA: ⑥ Bearer TR で呼び出し
    DA-->>FA: ⑦ {callerType: "app_only" / "delegated_agent_user",<br/>callerOid: "...", callerUpn: "..."}
    FA-->>BE: ⑧ LLM 整形レスポンス
    BE-->>FE: ⑨ 結果返却
    FE-->>U: ⑩ 結果表示
```
...
2. **Frontend の 2 トークン取得（Interactive のみ）**
   ...
   - **Foundry API トークン**：`https://cognitiveservices.azure.com/.default` スコープ（Foundry API 認証）

   MSAL の `acquireTokenSilent` を 2 回呼び出すことで実現できる。

3. **Tc のメッセージペイロード渡し**
   Interactive Flow では SPA が Foundry Agent API を直接呼び出し、Tc をメッセージペイロードに埋め込んで Function Tool に渡す。Backend API は Interactive Flow に関与しない。デモとして許容範囲内であり、短寿命の Tc（通常 1 時間）がそのまま使われるため、サーバーサイドでのセッション管理は不要。本番ではセキュアなセッション管理での渡し方を検討する。
...
- シナリオ選択（Interactive / Autonomous App / Autonomous User）
...
│  Entra Agent ID デモ                                     │
├───────────────────┬────────────────────┬─────────────────┤
│ 🔵 Interactive    │ 🔴 Autonomous App  │ 🟣 Autonomous   │
│    Flow           │    Flow            │    User Flow    │
├───────────────────┴────────────────────┴─────────────────┤
│ [シナリオ説明パネル]                                     │
│ "あなたの権限でエージェントが動く" 等                    │
...
│  📊 トークンフロー可視化                                 │
│  ○ Step 1: ユーザー認証        ✓ 完了                    │
│  ○ Step 2: OBO トークン取得    ⏳ 実行中...              │
│  ○ Step 3: API 呼び出し        ─ 待機中                  │
│                                                          │
├──────────────────────────────────────────────────────────┤
│  🤖 Foundry Agent の応答                                 │
│  ┌─────────────────────────────────────────────────────┐ │
│  │ あなた (alice@contoso.com) の委任権限でリソース API │ │
│  │ にアクセスしました。API はあなたのユーザー ID を認  │ │
│  │ 識し、delegated アクセスとして記録します。          │ │
│  └─────────────────────────────────────────────────────┘ │
├──────────────────────────────────────────────────────────┤
│  🔒 リソース API 生レスポンス                            │
...
| 状態               | 説明                                                              |
| ------------------ | ----------------------------------------------------------------- |
| `selectedScenario` | 選択中のシナリオ (interactive / autonomous_app / autonomous_user) |
| `authState`        | 認証状態 (not_logged_in / logged_in)                              |
| `demoState`        | デモ実行状態 (idle / running / completed / error)                 |
| `flowSteps`        | トークンフロー各ステップの進捗                                    |
| `agentResponse`    | Foundry Agent の LLM 整形レスポンス                               |
| `rawApiResponse`   | リソース API の生レスポンス                                       |
...
POST /api/demo/autonomous/app
  Body: {}
  → Foundry Agent に Conversation を作成（user token不要）
  → 同期 → { agentResponse, rawApiResponse }

POST /api/demo/autonomous/user
  Body: {}
  → Foundry Agent に Conversation を作成（user token不要）
  → 同期 → { agentResponse, rawApiResponse }
```

> **補足**: `azure-ai-projects` の `responses.create()` は blocking（同期）呼び出しのためポーリング不要。全エンドポイントは 1 リクエスト → 完全レスポンスの同期モデルを採用する。
>
> **Interactive Flow は Backend API を経由しない**: Interactive Flow では SPA が MSAL で取得した Foundry API トークンで Foundry Agent API を直接呼び出す。Tc はメッセージペイロードに埋め込んで Function Tool に渡す。これにより Backend API での Tc セッション管理（Redis 等）は不要となる。
...
> - Autonomous App → client_credentials で TR の subject は Agent Identity 自身
> - Autonomous User → user_fic により Agent User の upn が TR に引き継がれる
>
...
└── Agent Application: demo-entra-agent-id (Published)
    ...
    └── Function Tools:
        ├── call_resource_api_interactive(tc)
        │     → OBO (Tc の subject = 人間ユーザー) → TR で Resource API を呼ぶ
...
**`call_resource_api_interactive(tc: str)`**

```python
# 1. Tc はメッセージペイロードから直接受け取る（SPA → Foundry Agent 直接呼び出し）
...
```json
{
  ...
  "caller": {
    ...
    "appDisplayName": "Demo Client App",
    "scopes": ["CallerIdentity.Read"],
    "roles": [],
    "issuer": "https://login.microsoftonline.com/{tenantId}/v2.0",
    "issuedAt": "2026-03-26T09:59:00Z",
    "expiresAt": "2026-03-26T10:59:00Z"
  },
...
sequenceDiagram
    ...
    FE->>EID: acquireTokenSilent<br/>(scope: cognitiveservices.azure.com/.default)
    ...
    FE->>FA: Foundry Agent API 呼び出し<br/>[Authorization: Bearer {Foundry API token}]<br/>[message: "run_interactive tc={Tc_value}"]

    Note over U,DA: Phase 4: Foundry Agent によるトークン取得と API 呼び出し
    FA->>FA: call_resource_api_interactive(tc) を呼び出す
    ...
    DA->>FA: { callerType: "delegated_human_user",<br/>upn: "alice@contoso.com", ... }

    Note over U,DA: Phase 5: LLM 整形と結果返却
    FA->>FA: LLM が結果を自然言語で整形
    FA-->>FE: LLM 整形レスポンス + rawApiResponse
    FE->>U: 結果表示
```
...
```json
{
  ...
  "humanReadable": "alice@contoso.com の委任権限でアクセスされました"
}
```
...
sequenceDiagram
    ...
    U->>FE: Autonomous App シナリオを選択・実行を押す
    FE->>BE: POST /api/demo/autonomous/app

    BE->>FA: Conversation 作成<br/>message: "run_autonomous_app"
    ...
    FA->>DA: GET /api/resource<br/>Authorization: Bearer TR
    DA->>DA: TR 検証・claims 抽出
    DA->>FA: { callerType: "app_only",<br/>appId: "AgentIdentityID", ... }

    FA->>FA: LLM 整形
    FA->>BE: 整形済みレスポンス + rawApiResponse
    BE->>FE: { agentResponse, rawApiResponse }
    FE->>U: 結果表示
```
...
```json
{
  "caller": {
    ...
    "appDisplayName": "Demo Autonomous App Agent",
...
sequenceDiagram
    ...
    U->>FE: Autonomous User シナリオを選択・実行
    FE->>BE: POST /api/demo/autonomous/user

    BE->>FA: Conversation 作成<br/>message: "run_autonomous_user"
    ...
    FA->>DA: GET /api/resource<br/>Authorization: Bearer TR
    DA->>DA: TR 検証・claims 抽出
    DA->>FA: { callerType: "delegated_agent_user",<br/>upn: "agentuser@contoso.com", ... }
...
```json
{
  ...
  "humanReadable": "Agent User (agentuser@contoso.com) の委任権限でアクセスされました"
}
```
...
| **Agent User**                    | `agentuser@contoso.com`         | 手動作成                       | Autonomous User Flow で impersonate される |
...
Autonomous App:    Blueprint creds → T1, T1 → client_credentials → TR
                   TR の sub = Agent Identity 自身の OID ← サービスプリンシパルとして現れる

Autonomous User:   Blueprint creds → T1, T1→T2, T1+T2+username → user_fic → TR
...
>
> ただし、Foundry が自動生成した Blueprint に対して **Interactive Flow 用の `access_agent` スコープ（identifier URI と oauth2PermissionScopes）を追加設定**することは引き続き必要。これは Foundry の自動セットアップ対象外である（[create-blueprint](https://learn.microsoft.com/en-us/entra/agent-id/identity-platform/create-blueprint) 参照）。
...
| **Frontend UI**       | shadcn/ui + Tailwind CSS       | コンポーネントをコードとしてプロジェクトに取り込む設計のため、トークンフロー可視化パネル等のデモ固有 UI を細かくカスタマイズしやすい。Tailwind CSS でステップのハイライト表示を柔軟に実装できる               |
...
| **HTTP クライアント** | httpx (Python)                 | FastAPI の `async def` と組み合わせてノンブロッキングな HTTP リクエストを実装できる。`requests` とほぼ同一の API のため学習コストが低く、Foundry Agent API・Identity Echo API 両方の呼び出しに使用            |
| **Identity Echo API** | Python FastAPI                 | Backend API と同一スタックで運用を統一できる。JWT 検証ロジック（`python-jose`）を Backend と共有モジュールとして再利用可能                                                                                    |
| **JWT 検証**          | python-jose + cryptography     | Entra ID が発行する RS256 署名 JWT の検証に必要。JWKS エンドポイントから取得した公開鍵で署名を検証し、`aud` / `iss` / `exp` クレームを安全に検証できる。`callerType` 判定ロジックに直接活用                   |
...
| ドキュメント                               | URL                                                                                                |
| ------------------------------------------ | -------------------------------------------------------------------------------------------------- |
| Microsoft Entra Agent ID 概要              | https://learn.microsoft.com/en-us/entra/agent-id/overview                                          |
| Entra Agent ID 認証プロトコル概要          | https://learn.microsoft.com/en-us/entra/agent-id/identity-platform/agent-oauth-protocols           |
| Interactive Agent OBO フロー               | https://learn.microsoft.com/en-us/entra/agent-id/identity-platform/agent-on-behalf-of-oauth-flow   |
| Autonomous Agent App Flow                  | https://learn.microsoft.com/en-us/entra/agent-id/identity-platform/agent-autonomous-app-oauth-flow |
| Autonomous Agent User Flow                 | https://learn.microsoft.com/en-us/entra/agent-id/identity-platform/agent-user-oauth-flow           |
| Agent Identity Blueprint の概念            | https://learn.microsoft.com/en-us/entra/agent-id/identity-platform/agent-blueprint                 |
| Agent Identity の概念                      | https://learn.microsoft.com/en-us/entra/agent-id/identity-platform/agent-identities                |
| Blueprint の作成方法                       | https://learn.microsoft.com/en-us/entra/agent-id/identity-platform/create-blueprint                |
| Agent Identity の作成方法                  | https://learn.microsoft.com/en-us/entra/agent-id/identity-platform/create-delete-agent-identities  |
