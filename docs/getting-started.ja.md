...
### 2-2. Terraform の実行（Phase 1 — Foundry のブートストラップ）

`terraform apply` は **2 段階** に分けて実行します。
Phase 2 で作成される SPA App Registration が Blueprint の `access_agent` OAuth2 スコープを参照しますが、
このスコープはセクション 5 の `set-blueprint-scope.py` を実行するまで存在しません。
そのため Phase 1 では先に Foundry リソース（Agent Identity Blueprint も自動作成される）を作成し、
スコープを追加した後にセクション 6 の最終 apply を行います。

```bash
cd src/infra
terraform init
terraform plan
# Phase 1: Foundry Resource / Project / Capability Host / Model Deployment を作成。
# Seed agent ステップを通じて Agent Identity Blueprint が暗黙的に作成されます。
terraform apply -target=terraform_data.seed_agent
cd ../..
```

> **なぜ `-target` が必要?** SPA App Registration (`azuread_application.client_spa`) は plan 時に
> Blueprint の `access_agent` スコープを読み取ります。セクション 5 の `set-blueprint-scope.py` で
> スコープが追加される前にフル apply を実行すると、空配列に対する `Invalid index` エラーで失敗します。
> seed agent を target にすることで、Blueprint がプロビジョニングされた直後で Phase 1 を停止します。

Terraform は 2 つの Phase を通じて以下のリソースを作成します:
...
Entra Agent ID のセットアップスクリプト（セクション 5 と 7）は、Graph API の委任スコープ
...
`sync-infra-env.py` は `src/infra` と `labs/entra-agent-id/prereqs` の `terraform output` から
約 16 個の環境変数（prereqs アプリの `GRAPH_API_OPS_CLIENT_ID` を含む）を読み取り、
`src/.env` の該当行を上書きします。
手動で値を埋める必要はありませんが、**`.env` ファイルが事前に存在する必要があります**。
設定される変数の一覧は [環境変数リファレンス](#環境変数リファレンス) を参照してください。

> **Note**: Phase 1 完了時点では一部の output（例: `ENTRA_SPA_APP_CLIENT_ID`、Container App の URL 等）
> はまだ未生成のためスキップされます — これは想定動作です。Phase 2（セクション 6）後に本スクリプトを
> 再実行すれば、これらの値が埋められます。
>
...
## 5. Blueprint スコープの設定（Phase 2 apply の前提条件）

最終的な Terraform apply を実行する前に、Agent Identity Blueprint に `access_agent` OAuth2 スコープを公開します。
Phase 2 で作成される SPA App Registration がこのスコープを参照するため、事前に存在している必要があります。

このスクリプトは Blueprint に App ID URI (`api://{blueprint-client-id}`) と `access_agent` スコープを設定します。
SPA は後ほど `api://{blueprint}/access_agent` スコープを要求し、Interactive (OBO) フローで使用するユーザートークン (Tc) を取得します。
...
- `src/.env` から `ENTRA_AGENT_BLUEPRINT_IDENTITY_CLIENT_ID` と `GRAPH_API_OPS_CLIENT_ID` を読み込み
- べき等 — 既に設定済みならスキップ
- `--delete` オプションで設定を元に戻せる

---

## 6. Terraform の実行（Phase 2 — 残りのリソース）

Blueprint に `access_agent` スコープが公開されたので、SPA App Registration および残りの全リソースを作成できます。

```bash
cd src/infra
terraform apply   # 残りのリソースをプロビジョニング
cd ../..
```

> **Phase 2 の初回 apply** では Container Apps (Backend API / Identity Echo API) のコンテナイメージも
> ACR にビルド・プッシュされ、Container Apps へデプロイされます。

Phase 2 で作成される SPA Client ID と Container App URL を取り込むため、env 同期を再実行します:
...
## 7. Entra Agent ID のセットアップ

Foundry Project、Agent Identity Blueprint、および Agent Identity はすでに存在しています（セクション 2 で作成済み）。
残りの設定により、各 OAuth フローを動作させます。
...
### 7-1. Blueprint FIC の設定（全フロー共通・必須）
...
### 7-2. Interactive Agent (OBO) フロー用の設定

Interactive フローでは、ユーザーが SPA でログインし、Hosted Agent がユーザーの委任権限 (delegated) で
Identity Echo API にアクセスします。

> **Note**: Blueprint スコープ (`access_agent`) は Phase 2 apply の前提条件としてセクション 5 で
> すでに設定済みです。
...
### 7-3. Autonomous Agent (App) フロー用の設定
...
### 7-4. Autonomous Agent (User) フロー用の設定
...
| `set-blueprint-scope.py`                   | 全フロー（前提）      | Blueprint に App ID URI + スコープを公開（Phase 2 apply 前にセクション 5 で実行） |
