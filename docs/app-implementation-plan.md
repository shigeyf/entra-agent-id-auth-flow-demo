...
| **作成日**         | 2026-03-27                                                         |
| **最終更新日**     | 2026-04-05                                                         |
| **ステータス**     | Phase 2 Step C 完了（Backend API クラウドデプロイ + E2E 検証済み） |
...
│   ├── Dockerfile
│   └── routes/
│       └── demo.py           # POST /autonomous/app + /autonomous/app/stream
...
- [ ] `src/frontend/src/api/demoApi.ts` を新規作成し `runAutonomousApp()` 関数を追加（`POST /api/demo/autonomous/app/stream` を SSE で呼び出す、認証トークン不要）
...
- [ ] `src/agent/runtime/config.py` に `ENTRA_AGENT_ID_USER_UPN` 環境変数を追加
- [ ] `src/agent/agent.yaml` の `environment_variables` に `AGENT_USER_OID` と `ENTRA_AGENT_ID_USER_UPN` を追加
...
- [ ] デプロイ後確認: `curl https://{fqdn}/health` → `{"status": "ok"}`、`curl https://{fqdn}/api/resource` → 401

**Step B 以降:**

- [ ] Hosted Agent の `RESOURCE_API_URL` を Container App の FQDN に更新し Agent version を再作成
- [ ] Foundry Agent API 経由で `call_resource_api_autonomous_app` の E2E テスト
...
| Foundry Hosted Agent | `FOUNDRY_MODEL_DEPLOYMENT_NAME`            | モデルデプロイメント名（例: `gpt-4.1`）                                     |
...
> **Vite 環境変数の注記**: SPA は `VITE_` プレフィックスではなく、`vite.config.ts` の `envPrefix: ['ENTRA_', 'RESOURCE_API_', 'FOUNDRY_']` で定義されたプレフィックスを使用する。`.env` ファイルは `src/` ディレクトリに配置し、全コンポーネントで共有する（`envDir: '../'`）。Phase 5 で追加予定の Frontend 変数（Backend API URL、Blueprint App ID、Foundry Agent Endpoint）は実装時に追記する。
