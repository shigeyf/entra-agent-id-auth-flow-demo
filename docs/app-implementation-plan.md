...
- [x] `src/identity_echo_api/config.py` — 環境変数（`TENANT_ID`、`RESOURCE_API_CLIENT_ID`） — ✅ `.env` 設定済み
...
def check_agent_environment() -> str:
    ...
    return json.dumps({
        ...
        "env_project_endpoint": os.getenv("PROJECT_ENDPOINT", "NOT SET"),
        "env_blueprint_client_id": os.getenv("BLUEPRINT_CLIENT_ID", "NOT SET"),
        "env_agent_identity_oid": os.getenv("AGENT_IDENTITY_OID", "NOT SET"),
    }, indent=2)
```

**コーディング（`src/agent/`）:**

- [x] `src/agent/main.py` — ✅ `Agent` + `from_agent_framework(agent).run()` エントリポイント。SDK `azure-ai-agentserver-agentframework 1.0.0b17` で `Agent`（旧称 `ChatAgent`）、`tool`（旧称 `@ai_function`）に更新
- [x] `src/agent/tools/debug.py` — ✅ `check_agent_environment()` Function Tool（`@tool` デコレータ）
- [x] `src/agent/config.py` — ✅ `AgentConfig` dataclass（全環境変数定義済み）
- [x] `src/agent/requirements.txt` — ✅ `azure-ai-agentserver-agentframework`, `azure-identity`, `httpx`, `python-dotenv`
- [x] `src/agent/Dockerfile` — ✅ `FROM --platform=linux/amd64 python:3.11-slim`、`EXPOSE 8088`
...
| サーバーが起動しない                      | `PROJECT_ENDPOINT` 未設定、`MODEL_DEPLOYMENT_NAME` が存在しないモデル名 |
...
- [x] 環境変数（`PROJECT_ENDPOINT`, `BLUEPRINT_CLIENT_ID` 等）が正しく設定されていることを確認 — ✅ 全環境変数がコンテナ内で参照可能
...
**目的**: Step A-3 で「MSI が動く Hosted Agent」が確立した状態から、T1/TR Token Exchange を実装し、Identity Echo API を呼び出す。この段階で Backend API はまだ使わず、Agent の Function Tool から直接 Resource API を呼ぶ。

**コーディング（`src/agent/` に追加）:**
...
- [ ] `src/agent/main.py` — ツールリストに `call_resource_api_autonomous_app` を追加（`check_agent_environment` も残す）
...
| Identity Echo API    | `RESOURCE_API_CLIENT_ID`      | `demo-identity-echo-api` App Registration の Client ID                           |
| Identity Echo API    | `AGENT_USER_UPN`              | `agentuser@contoso.com`（callerType 判定用、Phase 4 以降）                       |
| Backend API          | `FOUNDRY_AGENT_ENDPOINT`      | Microsoft Foundry のエージェントエンドポイント URL                               |
...
| Foundry Hosted Agent | `RESOURCE_API_SCOPE`          | `api://{RESOURCE_API_CLIENT_ID}/CallerIdentity.Read`（delegated 用）             |
| Foundry Hosted Agent | `RESOURCE_API_DEFAULT_SCOPE`  | `api://{RESOURCE_API_CLIENT_ID}/.default`（app-only `client_credentials` 用）    |
