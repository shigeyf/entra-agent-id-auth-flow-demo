...
#
# Grants an Admin Consent (consentType: AllPrincipals) so the Agent Identity
# can act on behalf of ANY human user when accessing a resource API via OBO.
#
# This is required for the Interactive OBO flow. Unlike the Autonomous User
# flow (which uses consentType: Principal for a single Agent User), the OBO
# flow needs AllPrincipals because any logged-in user can trigger the flow.
#
# Reference:
...
#
# Usage:
#   python src/agent/entra-agent-id/grant-admin-consent-to-agent-identity.py            # grant
#   python src/agent/entra-agent-id/grant-admin-consent-to-agent-identity.py --delete   # revoke
...
def find_existing_admin_grant(headers: dict, client_id: str, resource_id: str) -> dict | None:
    """Find an existing AllPrincipals oauth2PermissionGrant."""
    resp = requests.get(
        f"{GRAPH_BASE}/oauth2PermissionGrants",
        params={
            "$filter": (
                f"clientId eq '{client_id}' "
                f"and consentType eq 'AllPrincipals' "
...
def grant_admin_consent(args: argparse.Namespace) -> None:
    ...
    print("Consent Type: AllPrincipals (Admin Consent)")

    # Check if grant already exists
    existing = find_existing_admin_grant(headers, agent_identity_id, resource_sp_id)
    if existing:
        existing_scopes = existing.get("scope", "")
        if scope_name in existing_scopes.split():
            print(
                f"\n✅ Admin consent grant already exists (id: {existing['id']}, "
        ...
        print(
            f"\n🔧 Updating existing grant (id: {existing['id']}) "
        ...
        if resp.status_code == 204:
            ...
            print(f"❌ Failed to update grant (HTTP {resp.status_code})")
            print(json.dumps(resp.json(), indent=2))
            sys.exit(1)
        return

    # Create new grant — AllPrincipals (no principalId)
    body = {
    ...
    print("\n🔧 Creating oauth2PermissionGrant (AllPrincipals) ...")
    ...
    if resp.status_code in (200, 201):
        ...
        print(f"   consentType: {grant.get('consentType')}")
        print(f"   resourceId:  {grant.get('resourceId')}")
        print(f"   scope:       {grant.get('scope')}")
    else:
        print(f"❌ Failed to create consent grant (HTTP {resp.status_code})")
        print(json.dumps(resp.json(), indent=2))
        sys.exit(1)


def revoke_admin_consent(args: argparse.Namespace) -> None:
    agent_identity_id = require_env("ENTRA_AGENT_IDENTITY_CLIENT_ID")
    resource_api_client_id = require_env("ENTRA_RESOURCE_API_CLIENT_ID")
    ...
    existing = find_existing_admin_grant(headers, agent_identity_id, resource_sp_id)
    if not existing:
        print("ℹ️  No admin consent grant found. Nothing to revoke.")
        return

    grant_id = existing["id"]
    print(f"🔧 Revoking admin consent grant (id: {grant_id}, scope: '{existing.get('scope')}') ...")
    resp = requests.delete(f"{GRAPH_BASE}/oauth2PermissionGrants/{grant_id}", headers=headers)

    if resp.status_code == 204:
        print("✅ Revoked successfully.")
    else:
        print(f"❌ Failed to revoke grant (HTTP {resp.status_code})")
        print(json.dumps(resp.json(), indent=2))
        sys.exit(1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Grant or revoke Admin Consent (AllPrincipals) for the Agent Identity "
        "to act on behalf of any human user via OBO flow."
    )
    parser.add_argument(
        "--scope",
        type=str,
    ...
    parser.add_argument(
        "--delete",
        action="store_true",
        help="Revoke the admin consent grant instead of creating it.",
    )
    args = parser.parse_args()

    if not args.delete and not args.scope:
        print(
            "ERROR: --scope is required (or set ENTRA_RESOURCE_API_SCOPE in .env)",
            file=sys.stderr,
        )
        sys.exit(1)

    if args.delete:
        revoke_admin_consent(args)
    else:
        grant_admin_consent(args)
